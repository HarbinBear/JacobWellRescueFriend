...
## 九、Playdead 方法论对应的技术落地稿（P6 / P7 / P8 / 曝光稳定性）

这一节用于把设计文档里的“Playdead 方法论在本项目中的表现落地稿”，进一步转成**后续 AI 可以直接照着实施**的技术说明。
...
如果后续任务涉及 `P6`、`P7`、`P8`、自动曝光、迷宫材质响应深化，建议先读本节，再开始动代码。

### 9.1 现状速记：当前相关系统已经在哪里

#### 9.1.1 `P6` 当前落点：VPL 仍然是离散点上传
...
#### 9.1.2 曝光当前落点：`WebGLLight.ts` 的启发式自动曝光
...
### 9.2 推荐总顺序：为什么先做 `P6` 和曝光，再做 `P7`，最后做 `P8`
...
- `src/render/RenderLight.ts`
  - 主题对反光和环境感知的影响
- `src/render/DustMotes.ts` 或 `src/logic/Particle.ts`
  - 主题对颗粒密度 / 颜色 / 散射的影响
