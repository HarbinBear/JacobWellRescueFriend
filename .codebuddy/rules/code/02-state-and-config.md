...
项目提供了一个 GM 工具面板（`src/gm/GMPanel.ts`），可以在运行时直接读写 `CONFIG` 对象中的参数。

面板入口是屏幕顶部中央的 GM 小圆圈按钮，点击打开后包含三个 Tab 页签：
...
- **Debug**：`debug`、`debugSpeedMultiplier`、`bShowNpcFlashLight`、`fishArenaMode`
...
如果需要新增可调参数，只需在 `GMPanel.ts` 的 `TABS` 数组中添加条目即可。
