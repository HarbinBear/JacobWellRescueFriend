...
- 新增 **SFX-Loop 通道**（区别于已有的一次性 SFX 与 BGM 通道）：`type SFXLoopKey = 'breathLoop'`，`SFX_LOOP_ENTRIES` 保存 `currentVolume / targetVolume / desiredPlay / playbackRate / fadeStep` 等运行态。
- 对外接口：
  - `playSFXLoop(key)`：启动循环（idempotent，重复调用不会重新 seek）。
  - `stopSFXLoop(key)`：把 `desiredPlay` 置 false，`targetVolume` 清零；`updateSFXLoops()` 中淡到 0 后真正 `pause()` 节省资源。
  - `setSFXLoopParams(key, { targetVolume?, playbackRate? })`：运行时调参数。`targetVolume` 会被 `CONFIG.audio.sfxVolume` 上限裁剪，`playbackRate` 被 clamp 到 0.5~2.0。
- `updateSFXLoops()`：独立导出，由 `game.ts` 主循环在 `updateAudio()` 之后调用，每帧逼近目标音量 + 写 `playbackRate`（`InnerAudioContext` 上此属性不一定生效，浏览器兜底路径生效）。
...
- 玩家或 NPC 潜水员绘制
  - 先看 `RenderDiver.ts`
- 呼吸气泡（世界层）/ 呼吸音（SFX-Loop）
  - 逻辑先看 `src/logic/BreathSystem.ts`
  - 气泡绘制先看 `src/render/RenderBreath.ts`
  - SFX-Loop 通道先看 `src/audio/AudioManager.ts` 中 `playSFXLoop / stopSFXLoop / setSFXLoopParams / updateSFXLoops`
  - 参数先看 `CONFIG.breath`，GM 面板"呼吸"Tab
  - 注意：气泡绘制必须在 `drawDustDarkLayer` 之后、世界 transform `ctx.restore` 之前（光照之前），否则黑暗区气泡会一样亮
