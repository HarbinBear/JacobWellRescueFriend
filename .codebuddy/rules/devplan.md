...
| **P14** | **场景图鉴物件系统** | 🟡 中 | ✅ 已完成 | 往迷宫场景里撒 10 种静态"遗落物"（骸骨/锈蚀硬币/陶罐碎片/小铁锚/银色指环/刻字石板/锈蚀鱼钩/小铜铃/锈蚀钥匙/螺旋海螺），玩家靠近并被手电光锥照到即"发现"，结算页展示本次新发现与图鉴累计进度。**设计边界**：(a) 绝对不可交互——不显示轮盘按钮、不调起任何 hint tip，避免玩家以为能捡；(b) 全部静态视觉——不发光、不呼吸、不脉冲、不闪烁、不会动，与氧气瓶/NPC/标记的视觉语言严格区分；(c) 已发现与未发现外观完全一致（故意不做高亮，避免二次视觉噪音）；(d) 尺寸 14~24px，贴岩石或散落地面，尺度远小于氧气瓶。**生成**：派生子 seed `mainSeed ^ 0x5EED1CE0`（与 fishDens `^0xDEADBEEF` / oxygenTanks `^0xCAFEBABE` 独立序列），默认每张迷宫 15 个（`CONFIG.relic.totalCount` 可调 10~20），50% 贴墙法线外缘、50% 散落在岩石附近通路上，类型按权重抽签（骨骸/石板稀有，硬币/海螺常见），位置需通过 `checkMazeCollision` 校验不嵌岩石，两两 ≥ 220px 间距，离出生点 ≥ 400px。**发现判定**：每帧 `updateRelicDiscovery()` 遍历 relics，未发现的只要 (a) 手电开着 + (b) 距玩家 < 110px + (c) 落在手电光锥 ±30° FOV 内，同时满足即记入 `discoveredRelicIds`（跨下潜累计）与 `_thisDiveNewRelicIds`（本次下潜专用）；图鉴进度跨下潜持久，但本次新发现列表每次 `startMazeDive` 清空。**结算页展示**：debrief.ts 在"新发现主题"之后追加"本次发现：古老骸骨×1、锈蚀硬币×2、螺旋海螺"行（按名字归并去重），并在底部灰字显示"本关图鉴进度 X / 15"。**渲染层**：新建 `src/render/RenderRelic.ts`，10 个绘制函数全部纯矢量（贝塞尔 / arc / ellipse / 几何线段），调色板统一"被水侵蚀"色调（冷象牙白骨、青铜、陶土、锈铁、磨银、墨绿铜锈、石灰灰、黄铜、海螺米黄）；绘制入口 `drawRelicsWorld(ctx, viewL, viewR, viewT, viewB)` 带视椎剔除，挂在 `Render.ts` 迷宫 play 阶段 `drawOxygenTanksWorld` 之后、`drawDustDarkLayer` 之前，与岩石/绳索/鱼同层，会被光照遮罩压暗（黑暗区物件不会自发光）。**存档**：`state.mazeRescue.relics` 运行时数组进 rest 黑名单（靠 seed 重建）；`state.mazeRescue.discoveredRelicIds` 自动走 rest 持久化；`diveHistory[*].newRelicIds` 经由 `PackedDive` 新增字段跨 session 保留（读档分支用 `(d as any).newRelicIds` 兜底老存档）；`clearMazeSave` 触发新地图时重新生成 relics 并清空 discoveredRelicIds（体现"换关=全新图鉴"）。**文件改动**：新建 `src/logic/Relic.ts`（约 220 行：类型表 + 派生种子生成 + 每帧发现判定 + 对外查询）+ `src/render/RenderRelic.ts`（约 320 行：10 个 drawXxx 纯矢量 + 总入口）；改动 `src/core/state.ts` 给 mazeRescue 加 `relics / discoveredRelicIds`；`src/core/config/gameplay.ts` 加 `CONFIG.relic` 子对象（totalCount / onWallRatio / minDistBetween / minDistToSpawn / discoverRadius / discoverFovDeg 6 参数）；`src/logic/MazeLogic.ts` 两个生成分支调 `buildRelicsForMaze`、读档分支重建 relics、`updateMaze` 调 `updateRelicDiscovery`、`startMazeDive` 调 `resetRelicDiscoveryForDive`、`finishMazeDive` 把 `getThisDiveNewRelicIds()` 塞进 diveHistory；`src/logic/MazeSave.ts` rest 黑名单加 `relics`、`PackedDive` 新增 `newRelicIds` 字段、pack/unpack 两端都带着；`src/render/Render.ts` 新增 `drawRelicsWorld` 调用；`src/render/mazeUI/debrief.ts` 结算页追加"本次发现 + 图鉴进度"两行展示。**典型陷阱**：(a) 一定不能把 relic 扯上 `state.wheel` / `detectWheelContext` / 按钮 hit-test 等任何交互分支——它就是装饰+记录，不是物件拾取；(b) 绘制层必须在 `drawDustDarkLayer` 之前、世界 transform `ctx.restore` 之前，才能被光照遮罩压暗；放到 `drawDustLitLayer` 或 HUD 层都会导致黑暗区物件发亮破坏氛围；(c) `discoverFovDeg` 与 `CONFIG.fov` 不挂钩——故意用独立参数，FOV 默认 60° 比手电稍窄，让"照到"的判定比"看见"更严格；(d) `relics` 不写进 rest 存档这件事依赖 MazeSave 的黑名单匹配，若未来字段名改动要同步更新 `if (k === 'relics')` 分支。**第二版增强**（2026-05-09）：(A) **全部贴岩石外缘 D2 方案**——取消原 50% 散落分支，物件永远贴在岩石表面外缘 4~7px 处，不再悬浮通路。(B) **尺寸放大**——生成侧 size 从 0.9~1.1 调到 1.5~1.9，RenderRelic 再乘 RELIC_SCALE_MUL=1.1，实际像素尺寸 28~38px。(C) **发现瞬间世界层飘字**——Relic.ts 加模块级 _hintList 列表 + getRelicHints()，updateRelicDiscovery 记入发现那一帧 push '发现 · <物件名>'，90 帧寿命淡入淡出并上浮 0.35px/帧；RenderRelic 新增 drawRelicHintsWorld（米色小圆点 + 斜体 Georgia 文字 + 黑阴影）；Render.ts 把这个调用挂在 drawDustLitLayer 之后（不被光照压暗，保证黑暗边缘发现反馈可见）；resetRelicDiscoveryForDive 同步清空 hint。(D) **岸上图鉴入口 + 全屏图鉴页**——新建 src/render/mazeUI/codex.ts：drawCodexEntryBtn（右上角让开微信胶囊 90x34）+ drawMazeCodex（羊皮纸底 + 5x2 卡片网格 + 底部进度条，已发现彩色图+名+描述，未发现灰剪影+？？？）+ getCodexEntryBtnRect/getCodexCloseBtnRect；state.mazeRescue 加 codexOpen；RenderMazeUI 在 shore/resolved_idle 分支最前判 codexOpen 分发到全屏页；shore.ts 末尾追加 drawCodexEntryBtn；input.ts touchStart shore 分支 codexOpen 时提前 return 避免误长按，touchEnd 加点关/点打开两个分支。**本轮文件**：Relic.ts / RenderRelic.ts / Render.ts / mazeUI/codex.ts(新建) / RenderMazeUI.ts / mazeUI/shore.ts / state.ts / MazeLogic.ts / input.ts。**新陷阱**：(e) _hintList 模块级 let 不进存档；(f) drawRelicHintsWorld 必须在光照层之后；(g) codexOpen 走 rest 自动持久化，老存档兜底为 false。 |
| **P15** | **减压停留系统（DCS）** | 🟡 中 | ✅ 已完成 | 硬核向上浮节奏改造。将"秒出水面"改为"必须在 12/9/6/3m 逐档停留才能安全出水"，把氮气累积 / 减压停留 / DCS 失败分支一整套打通。**阶段 1（基础系统）**：新建 `src/logic/DecompressionSystem.ts` 单值 nitrogenLoad 吸排模型（深>20m 吸氮、深<10m 排氮、超装备极限再 ×2）+ 四档停留表（12/9/6/3m 分别停 3/5/8/12s）+ HUD 左上角新增"减压灯"图标（第 3 位）含绿/黄/红/深红四级视觉 + 正在做减压时显示蓝绿 progress 环 + 档位深度数字 + 长按加速 5× 速度 3× 耗氧 + CONFIG.deco 24 参数全量暴露在 GM 面板"减压" Tab + 8 个测试 action（decoSetYellow/Red/Critical/Clear、decoTriggerPenaltyLv1/2、decoClearPenalty、decoDump）+ 首次黄灯教学文案 + ExtractionState 新增 optional decoPenalty 字段 + Loadout 应用 O2Max -30% + Economy 按 lootMul 随机丢弃保留物品。**阶段 2（锁定模型修复）**：第一阶段上线后发现"直接上浮没问题，减压 UI 还自己重置到安全状态"的致命 bug——根因是任务会随氮负荷被浅水自然排氮自动清空，玩家漂到水面就绕过。修复方案：引入 `lockActive` 锁定位，首次进黄线即锁、只有完成全部档位才解锁，浅水漂泊**不会**解除任务；新增 `surfacingReason='deco'` 失败分支（ExtractReason 类型扩展、settle 同 o2/fishkill 全损、DebriefExtension reasonLabel 加映射、RenderMazeUI failed 分支紫色色罩 + "未完成减压 · 重度减压病"副文字、failed 物理分支 deco 抽搐下沉）；撤离长按完成 + 锁定中 → 直接判 deco 失败；救援到出口 + 锁定中 → 首次弹警告 + 滞留 5s 判 deco 失败（带着 NPC 也算失败）；`triggerDecoPenaltyOnSurface()` 改为只在 lockActive=true 时触发，未进黄线 / 已完成减压一律无惩罚；教学文案加强到"⚠ 氮气累积，必须做减压停留！出水前需在 12/9/6/3m 逐段停留，不完成减压直接出水 = 重度减压病"；HUD visible 加入 lockActive 判断锁定中始终可见；GM decoSet/Clear 同步重置 lockActive 避免调试被锁死，decoTrigger 反向模拟 lockActive=true。**DCS 惩罚**：severity 1（N₂<1.5）= O2Max -30% 持续 1 次下潜 + 战利品 50% 丢失；severity 2（N₂≥1.5）= O2Max -30% 持续 2 次 + 战利品 100% 丢失 + 紫标 debuff（isPurpleDebuffActive 已导出，岸上 UI 紫色徽章下一迭代接入）。**文件新建**：`src/logic/DecompressionSystem.ts`（~370 行）+ `.codebuddy/rules/design/extraction/08-decompression-system.md`（设计文档，README 索引已更新）。**文件修改**：`src/core/config/gameplay.ts`（新增 deco 子对象 24 参数）、`src/logic/MazeLogic.ts`（import + startMazeDive 重置 + updateMaze tick + finishMazeDive 惩罚 + 撤离/胜利检测加锁定判定 + failed 分支 deco 物理）、`src/extraction/core/ExtractionState.ts`（decoPenalty optional）、`src/extraction/logic/Loadout.ts`（O2Max 打折）、`src/extraction/logic/Economy.ts`（ExtractReason 加 deco + lootMul 随机丢物品）、`src/extraction/logic/ExtractionDive.ts`（switch 加 deco）、`src/extraction/render/DebriefExtension.ts`（reasonLabel 映射）、`src/render/HUDTopLeft.ts`（deco HUD 项 + drawDecoIcon）、`src/render/RenderMazeUI.ts`（failed 分支 deco 紫色画面）、`src/gm/GMConfig.ts`（减压 Tab）、`src/gm/GMPanel.ts`（7 个减压 action）。**典型陷阱**：(a) `lockActive` 是任务生命周期唯一真相，不能靠 nitrogenLoad 数值自动解锁，调 GM 改数值不会解锁；(b) `triggerDecoPenaltyOnSurface` 必须在 `onExtractionDiveEnd` 之前（让 settle 读 currentLootMul）、`consumeDecoPenaltyDive` 必须在之后（把本次算一次消耗），顺序反了就立刻清零；(c) `currentLootMul` 只用一次，consume 时置 undefined，避免下次下潜还打折；(d) 救援滞留时间戳 `_decoBlockedRescuedSince` 不进持久化（故意的，避免读档瞬间判死）；(e) **长按加速 × 3 耗氧尚未接入 BreathSystem**：`getDecoO2Mul()` 已导出但 `consumeBreathO2()` 还没乘，下次迭代需要在 BreathSystem 的 o2 消耗处乘此值才能完整实现"加速 = 更费氧"的设计意图。`npm run typecheck` 通过。详细设计见 [08-decompression-system.md](./design/extraction/08-decompression-system.md)、详细实现归档见 [09-completed-work-archive.md](./code/09-completed-work-archive.md) 末段。 |
| **P16** | **BCD 浮力控制系统（浮力背心）** | 🔴 高 | ✅ 第一版已完成 | 把"深度"从坐标变成需要持续经营的状态。此前浮力只有呼吸这一层（玩家不可控、幅度极小），上下移动与左右移动在操作上完全同质。本系统在其上叠加一层**玩家主动控制的粗调浮力**，与真实潜水"BCD 粗调 + 呼吸微调"的分工完全一致。**核心物理（玻意耳定律驱动）**：唯一状态量 `gasNL`（气囊内气体标准升，不随深度变化），每帧派生 `P = 1 + depth/10`、`volL = gasNL/P`、`suit = suitLiftSurfaceL/P`、`a_y = baseSinkAccel − (volL+suit)×liftAccelPerLiter`，加速度叠加进 `player.vy`。**中性所需体积随深度增大**（湿衣被压扁失效：水面 1.14L → 30m 1.51L），所以玩家下潜时会同时看到"气量条自己往下掉（1.34L→0.55L）+ 中性黄线自己往上爬"，两条线越拉越开——玻意耳的可视化，教学完全自解释。**充排气按实际体积流量、气瓶消耗按标准升**：前者保证"按住一秒"的手感与深度无关（可预期），后者让"深处充气更费气瓶"自动涌现（30m 同体积要 4 倍标准升），一举同时拿到操作手感与真实耗气纪律。**数值标定**（waterDrag=0.98 → 终端速度=加速度×50；tileSize=120px=1m → 1px/帧=30m/min）：排空水面 0.35 m/s 下沉 / 排空 30m 0.46 m/s（更沉）/ 量表满格 30m 18 m/min 上浮（危险阈值）/ 容量上限 30m 45 m/min（严重失控）；玩家游泳约 4 px/帧，浮力是"背景漂移"级别，必须持续关注但不剥夺操作权。**惩罚闭环刻意不新增失败分支**：失控上浮时每秒往 `nitrogenLoad` 灌 `ascentNitrogenPerSec`（新增 `DecompressionSystem.addNitrogenLoad()`），于是"忘记排气→越升越快→氮负荷飙升→减压任务锁定→不做完停留出水=重度减压病"这条链完全复用 P15，且因果关系与真实医学一致（上浮过快正是 DCS 首要诱因）。反向咬合：减压停留要求 `|vy|<0.8` 持续数秒，逼玩家必须先把 BCD 配平好才做得完减压。**交互（右侧竖直 inflator 手柄）**：原型来自真实装备（左肩垂下的充气管，拇指上下拨）。上钮＋充气/上浮、中间气囊量表、下钮－排气/下沉。三条不可妥协的约束：① 方向语义服从屏幕直觉而非真实倒握姿态（否则建立不起条件反射）；② 中性刻度线必须画成会动的（唯一教学载体，静态线会丢掉玻意耳这层）；③ 净浮力必须与气量分开显示（玩家真正要控的是"净浮力=0"而不是气量），量表右侧另挂中心为零的双向条，绿色向上=被托起/橙色向下=被拽沉，完全归中时整根手柄亮青色呼吸光环给正反馈。手感：按住持续、轻点=一个 short burst（`minBurstSec` 补足）、滑出即松手、两钮同按排气优先（真实处置原则先止住失控）、左手摇杆+右手排气可并行。位置避开左下撤离按钮/右下轮盘/左上 HUD 竖列/右上探知仪雷达点。**视觉分五层**：控件（按钮外扩脉冲光环+进气流短线抖动+排气气泡上飘 / 量表填充色按离中性远近变青绿-亮蓝-暗蓝灰+液面晃动高光+顶部溢流三角+底部 `x.x ATA` 压力读数）、世界层潜水员（`drawBCDBladder` 在躯干之下画气囊：气越多囊越鼓、边缘越亮、织带被撑开、左肩排气阀排气时亮起）、世界层气泡（`spawnBCDVentBubbles` 从左肩阀连续涌出一大串，比呼吸气泡更急更密更散）、全屏告警（失控上浮=上缘橙红脉冲+"⚠ 上浮过快/立即按 －排气·氮气正在飙升"；失控下沉=下缘琥珀脉冲+"下沉过快·按 ＋补气"，边缘渐变不挡视野中心）、音效（复用 `collisionBreath`=BreathBubble.mp3 作一次性嘶气声按 `sfxIntervalMs` 重复，充气 rate 1.7 尖锐小口径进气 / 排气 rate 1.25 闷粗大口径排水，零新增音频资源）。**教学节奏**：每次下潜前 5s 控件左侧浮小卡片（浮力背心/＋充气上浮/－排气下沉/黄线=中性）；首次深度超 12m 推一条剧情文案；之后全靠视觉自解释。**文件新建**：`src/logic/BCDSystem.ts`（~430 行）+ `src/render/RenderBCD.ts`（~530 行）+ `.codebuddy/rules/design/extraction/09-bcd-buoyancy-system.md`（设计文档，README 索引已更新）。**文件修改**：`src/core/config/gameplay.ts`（新增 `CONFIG.bcd` 37 参数）、`src/logic/MazeLogic.ts`（startMazeDive 重置 + updateMaze tick/叠加/教学 + o2 结算加 `consumeBCDO2()` + 被咬时释放双钮）、`src/logic/DecompressionSystem.ts`（新增 `addNitrogenLoad`）、`src/logic/BreathSystem.ts`（新增 `spawnBCDVentBubbles`）、`src/core/input.ts`（双钮触摸 start/move/end + 滑出释放 + 钮间切换 + 多点并行 + 全屏页/GM 抢占时 `bcdReleaseAll`）、`src/render/RenderMazeUI.ts`（play 分支绘制告警层+控件）、`src/render/RenderDiver.ts`（`DiverMotion` 加 `bcdInflation`/`bcdVenting` + `drawBCDBladder`）、`src/render/Render.ts`（传参）、`src/gm/GMConfig.ts`（BCD Tab 37 项 + 4 action）、`src/gm/GMPanel.ts`（bcdFill/bcdEmpty/bcdTrim/bcdDump）。**典型陷阱**：(a) `resetBCDSystem()` 必须在 `player.y` 写成出生深度之后调（`initialFillMode='neutral'` 要按入水深度反算中性气量），当前放在 `snapCameraToPlayer()` 之后；(b) `gasNL` 是唯一真相、`volumeL` 是派生量，GM 改状态必须同时写 `gasNL = volL × pressureAtm` 否则下一帧被冲掉；(c) 充排气按体积流量、气瓶消耗按标准升，两边同单位会丢掉"深处更贵"或"深处按一秒没反应"之一；(d) BCD 触点绝不能登记进 `manualDrive.activeTouches`，touchStart 命中必须 `continue`（否则想排气结果人游走了），但 touchMove 刻意**不** early-return 才能让"摇杆+排气"并行；(e) 全屏页/GM 抢占触摸后不会再回调 touchEnd，所有这类分支都要 `bcdReleaseAll()` 否则留下永久漏气的幽灵按住态；(f) 失控判定必须带 `netAccel` 方向条件，只看 `|vy|` 会把"主动往上游"误判成失控（游泳约 4 px/帧远大于 0.55 阈值）；(g) `drawBCDBladder` 必须画在躯干填充之前，画后面会盖住身体气瓶手臂像套了个塑料袋；(h) 不要给 BCD 单独做失败分支，惩罚全部经 `addNitrogenLoad` 流进减压系统，避免两套结算语言。**刻意留到下一版**：BCD 装备化（`bcd` 槽 + `bcdCapacityL`/`bcdDeflateMul` effects + 商店池，避免同时改经济系统）、可选配重（改 `baseSinkAccel`）、干衣双阀、卡阀故障事件（逼玩家长按 3s 断开 LPI 自救）、结算页展示 `getBCDO2SpentTotal()`。`npm run typecheck` 通过。详细设计见 [09-bcd-buoyancy-system.md](./design/extraction/09-bcd-buoyancy-system.md)。 |
...
P4（地形序列化）──→ 存档缓存需求（另提单）
P4（地形序列化）──→ 好友分享需求（另提单）
P16（BCD 浮力背心）──→ P15（减压系统，惩罚闭环经 addNitrogenLoad 复用它）
P16（BCD 浮力背心）──→ P3（呼吸浮力，BCD 粗调叠加在它的微调之上）
P16（BCD 装备化，下一版）──→ 撤离经济系统（装备槽 / 商店池 / Loadout）
```
...
- **场景图鉴系统第三版（总图鉴 32 种 + 成就感反馈）**（2026-05-09 本轮）：把 P14 图鉴系统从"每关 10 种、每关本地小图鉴"升级为"总图鉴 32 种、跨关累计、有成就感的真总图鉴"。
  - **32 种类型**：`Relic.ts::RelicKind` 联合类型从 10 种扩到 32 种。新增 22 种：人类遗物（silverCoin/humanSkull/pocketWatch/oilLamp/smallKnife/maskShard/waterFlask/ironNail/brassCompass/leatherBoot）、宗教符号（cross/amulet/idolFigure/crystal）、陶瓷日用（ceramicBowl/glassBottle）、海洋（coralChunk/sharkTooth/fishSkeleton）、地质（fossil/obsidian）、现代（cameraHousing）。`RELIC_TYPES` 表为每一种配中文名 + 简短叙事描述 + 出现权重（骸骨/头骨/石板/相机等稀有 weight=2~3，硬币/海螺/钉子常见 weight=8~10）。
  - **每关不重复（完全随机抽样）**：`Relic.ts` 新增 `pickUniqueKinds(n)`：用一次性"按权重抽签 + 抽完置零避免重复"的方式从 32 种抽 n（默认 15）种不重复 kind。`generateRelics()` 重写：先调 `pickUniqueKinds(totalCount)` 拿到本关 kind 列表，然后对每种独立尝试 80 次找合法位置（贴墙外缘 + 不撞岩石 + 离出生点 ≥ minDistToSpawn + 两两 ≥ minDistBetween），每种 kind 在单关只生成一个。完全随机抽样不看玩家是否已发现过该 kind（用户明确选 B 方案）。
  - **总图鉴跨关累计**：`state.mazeRescue` 新增两个字段 `codexKinds: string[]`（跨关累计的"已发现 kind 集合"）+ `codexSelectedKind: string | null`（图鉴页详情卡选中项，不持久化）。`MazeLogic.resetMazeLogic()` 在新建地图分支重建 mazeRescue 对象前先把旧的 codexKinds 暂存到局部 `preservedCodexKinds`，新对象建好后再恢复进去——这一步保证换新地图时 `relics`/`discoveredRelicIds` 重置但 `codexKinds` 不丢；读档分支兜底两个新字段（老存档缺失时填空数组 / null）。
  - **发现瞬间双反馈**：`Relic.ts::updateRelicDiscovery()` 增加"全新 kind 检测"：发现一个 relic 时同时维护 `codexKinds` 和模块级 `_thisDiveNewKinds`；如果该 kind 不在 `codexKinds` 里就额外推一条金色 `kind: 'newCodex'` 的 `_hintList` 项（"★ 新图鉴 +1"，120 帧寿命，位置 y-48 比普通发现 hint 更高）。`RenderRelic.ts::drawRelicHintsWorld` 按 hint.kind 分两路绘制：normal 走米色 Georgia，newCodex 走"金色辉光（shadow blur=10 + 金色发光）+ italic bold 15px Georgia + 进场 punch 缩放（前 0.2 期 1.35→1.0）"。普通发现仍保留之前的"发现 · XXX"米色文字。
  - **结算页金色高亮**：`Relic.ts` 新增 `getThisDiveNewCodexCount()`；`MazeLogic.finishMazeDive` 把它写入 `diveHistory[*].newCodexKindCount`；`MazeSave.ts` 的 `PackedDive` 新增 `newCodexKindCount?: number` 字段，pack/unpack 双向带上（老存档兜底 0）。`debrief.ts` 新增三行展示：(1) `本次发现：XX×2、YY` 普通文本，(2) `★ 本次新增图鉴 X 种 ★` 金色 italic bold + 金色 shadow（仅 newCodexCount > 0 时显示），(3) `图鉴进度 K / 32`（满档时变金色）；调用 `getCodexFoundKindCount() / getCodexTotalKindCount()` 直查总图鉴。
  - **图鉴页彻底重做（4×8 网格 + 里程碑 + 详情卡）**：`mazeUI/codex.ts` 整文件重写。布局：标题"水下图鉴" + 标题下波浪线 + 里程碑条幅（68~108px）+ 4 列 × 8 行格子区（128~ch-68）+ 底部金色进度条（ch-40）+ 返回按钮（左上）。里程碑表 `MILESTONES`：8 / 16 / 24 / 32 四档 → "探索初见 / 博物学家 / 资深考古 / 雅各布井档案官"（满级条幅有金色 linearGradient 渐变 + 两侧五星 + 金色描边 1.5px，未达条幅克制深褐底）。32 格按 ALL_RELIC_KINDS 顺序排：已发现彩色矢量 + 名字，未发现剪影 + "？？？" + 中央大问号；点单格设 `codexSelectedKind = kind` 弹中央详情卡（320×240，半透明背板压焦点 + 圆形大图区 + 物件名 + 描述 wrapText 三行），点详情卡任意处关详情，点返回按钮关整页，点其他空白不动作。底部金色 linearGradient 进度条（满级三段渐变 #b07a30→#f4d270→#b07a30 / 未满两段 #8a5a28→#cf9e48），上面叠四个里程碑刻度短线。
  - **岸上入口按钮升级**：`drawCodexEntryBtn` 文字改成显示总图鉴种类进度 `getCodexFoundKindCount() / getCodexTotalKindCount()` 而不是本关物件数，满档时按钮描边变金色 1.6px 宽。
  - **图标库**：`RenderRelic.ts` 世界层补 22 个 `drawXxx` 函数（对应 22 种新 kind 的纯矢量绘制）；`codex.ts` 内部补 32 个 `iconXxx`（图鉴页专用的小号简化版本，与世界层造型一致但去掉锈斑/阴影等小细节让小尺寸下更清晰）。共用调色板（bone/coin/pot/iron/silver/stone/brass/shell + rust/gem 等冷色系遗落物）。
  - **input 交互**：`input.ts` 在图鉴 codexOpen 分发段加优先级处理：先判 `codexSelectedKind` 有没有（若有 → 任意点击关详情卡），再判返回按钮，再遍历 32 个 `getCodexCellRectByIndex(cw, ch, i)` 判断点了哪格，命中后用静态 import 的 `ALL_RELIC_KINDS[i]` 拿 kind 写入 `codexSelectedKind`。其他空白点击不再关页（避免误关）。
  - **涉及文件**：`src/logic/Relic.ts`（重写 RelicKind/RELIC_TYPES/ALL_RELIC_KINDS/pickUniqueKinds/generateRelics + 新增 codexKinds 维护 + getCodexFoundKindSet/Count/TotalKindCount/getThisDiveNewCodexCount + 双类型 hint 推入）、`src/render/RenderRelic.ts`（switch 分发到 32 种 + 22 个 drawXxx + drawRelicHintsWorld 双路渲染 normal/newCodex）、`src/render/mazeUI/codex.ts`（整文件重写：4×8 网格 / 里程碑条幅 / 详情卡 / 进度条金色填充 / 32 个 iconXxx）、`src/render/mazeUI/debrief.ts`（图鉴展示三行升级）、`src/logic/MazeLogic.ts`（preservedCodexKinds 跨关恢复 + diveHistory 写 newCodexKindCount + import 补 getThisDiveNewCodexCount）、`src/logic/MazeSave.ts`（PackedDive 加 newCodexKindCount + pack/unpack 兜底 0）、`src/core/state.ts`（mazeRescue 加 codexKinds + codexSelectedKind）、`src/core/input.ts`（补两个新 getter import + 重写 codex 全屏页 touchEnd 分发 + 新增 ALL_RELIC_KINDS 静态 import）、`src/render/RenderMazeUI.ts`（re-export 补 getCodexCellCount/getCodexCellRectByIndex）。`npm run typecheck` 通过。
  - **典型陷阱**：(a) `Set<RelicKind>` 在 TS 严格模式下需要明确泛型；(b) `codexKinds` 跨关保留依赖于"在重建 mazeRescue 对象前先抓引用到局部变量"，否则覆写后旧引用即丢；(c) `newCodexKindCount` 不能在 debrief 通过"reverse 算 kind set 大小"反推（因为不知道 dive 开始前 codexKinds 状态），必须由 finishMazeDive 落盘到 diveHistory；(d) input.ts 不能用 `require('../logic/Relic')`，TS 目标 ES2016 + 微信小游戏 runtime 不支持 CommonJS require，必须静态 import；(e) 详情卡的"点任意处关详情"必须排在"点返回按钮"前面，否则在详情卡上点返回会先被详情关掉吃掉。
- **减压停留系统（DCS）**（2026-05-11）：把"上浮"从秒出水面改造为"必须按 12/9/6/3m 逐档停留才能安全出水"的硬核节奏。分两阶段上线：阶段 1 基础系统（新建 `src/logic/DecompressionSystem.ts` 单值 nitrogenLoad 吸排模型 + 4 档停留表 + HUD 减压灯 4 级视觉 + 长按加速 5×/3× 耗氧 + CONFIG.deco 24 参数 + GM"减压" Tab + DCS 惩罚写入 decoPenalty + Loadout O2Max 打折 + Economy 按 lootMul 随机丢物品）；阶段 2 修复"直接上浮没问题"的致命 bug（引入 `lockActive` 锁定位、首次进黄线即锁、只有完成全部档位才解锁、浅水漂泊不会解除任务、新增 `surfacingReason='deco'` 失败分支、撤离/胜利检测加锁定判定、RenderMazeUI failed 分支紫色画面 + "未完成减压 · 重度减压病"副文字、教学文案加强）。详细设计见 [design/extraction/08-decompression-system.md](./design/extraction/08-decompression-system.md)，详细实现归档见 [code/09-completed-work-archive.md](./code/09-completed-work-archive.md) 末段"减压停留系统（2026-05-11）"小节。

**下一步优先**：
1. P16 BCD 系统真机手感调优：`baseSinkAccel` / `liftAccelPerLiter` / `suitLiftSurfaceL` 三个参数决定"配平难度曲线"，需要真机实测后定档；控件位置在不同机型（尤其刘海屏 / 窄屏）上要复核不与轮盘按钮打架
2. P16 BCD 装备化（`bcd` 装备槽 + `bcdCapacityL`/`bcdDeflateMul` effects + 商店池 + `autoSelectBestEquipment` 的 pickBest 扩展）
3. P1 角色表现修复（T1.3 roll 滚动、T1.4 腿部脚蹼、T1.5 手电位置）
4. P4 地形序列化系统剩余任务（T4.4/T4.5/T4.6 MapCodec 编解码器——仅在需要做好友分享链接/二维码时再做；当前 wx.storage JSON 存档已够用）
5. P15 减压系统后续接线：(a) `BreathSystem.consumeBreathO2()` 返回值乘 `getDecoO2Mul()` 完成"长按加速 = 3× 耗氧"的闭环；(b) 岸上 UI 接入紫色徽章（`isPurpleDebuffActive()` 已就绪）；(c) 真机数值调优（吸氮速率 / 档位时长 / 惩罚强度）
