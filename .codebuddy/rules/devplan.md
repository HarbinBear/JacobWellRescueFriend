...
| P7 | 相机系统（远近自适应缩放） | 🔴 高 | 🟡 进行中 | 弹簧臂+摇曳已完成，需新增空旷/狭窄自适应缩放 |
...
- [ ] T7.8 实现多方向射线空间检测（约 10 条射线，均匀 360°）
- [ ] T7.9 根据射线结果计算空间开阔度指标
- [ ] T7.10 实现开阔度到目标 zoom 的映射（可配置曲线）
- [ ] T7.11 实现 zoom 平滑过渡（与现有弹簧臂系统协同）
- [ ] T7.12 GM 面板新增远近系统参数调试入口
- [ ] T7.13 真机测试与参数细调
...
**当前迭代**：P2 角色表现修复 + P7 远近系统 + P11 地形序列化
**已完成**：P1、P3、P4、P7 基础版、P8 第一版完整完成；P2 第一版完成；P5 第一版完成；废弃代码清理完成；迷宫 UI 重设计完成
**下一步优先**：
1. P2 角色表现修复（T2.10 roll 滚动、T2.11 腿部脚蹼、T2.12 手电位置）
2. P7 远近自适应缩放（T7.8 ~ T7.13）
