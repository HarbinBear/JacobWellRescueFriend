...
| 编号 | 需求名称 | 优先级 | 状态 | 备注 |
|------|----------|--------|------|------|
| P1 | 角色控制（手动挡模式） | 🔴 高 | ✅ 已完成 | 剩余 T1.8 真机测试 |
| P2 | 角色表现（重绘潜水员） | 🔴 高 | 🟡 进行中 | 剩余 T2.8 朝向验证、T2.9 真机细调 |
| P3 | 岩石生成一致性 | 🔴 高 | ✅ 已完成 | — |
| P4 | 水体渲染（悬浮尘埃） | 🔴 高 | ✅ 已完成 | — |
| P5 | 标记系统 | 🔴 高 | 🟡 进行中 | 剩余 T5.10 岸上认知地图显示标记 |
| P6 | 手电筒光照改进（VPL连续化） | 🟡 中 | ⬜ 未开始 | 离散虚拟光源 → 连续反射面 |
| P7 | 相机系统（弹簧臂+水中摇曳） | 🟡 中 | ✅ 已完成 | 剩余 T7.7 真机联动测试 |
| P8 | 浅水区渲染（阳光系统） | 🟡 中 | 🟡 进行中 | 剩余 T8.7 shader 融合、T8.8 真机细调 |
...
**当前迭代**：P11 地形序列化系统设计完成，准备开始实施
**已完成**：P1、P3、P4、P7 完整完成；P2 第一版完成；P5 第一版完成；P8 第一版完成；废弃代码清理完成
**下一步优先**：
1. P11 地形序列化系统（T11.1 ~ T11.10）
2. P5/T5.10 岸上认知地图显示标记
3. P8/T8.7 阳光与 shader 层深度融合
4. 各项真机测试（T1.8、T2.8、T2.9、T7.7、T8.8）
