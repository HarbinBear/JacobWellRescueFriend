...
| **P9** | **生命探知仪** | 🔴 高 | ✅ 第二版已完成 | 迷宫模式 play 阶段，NPC 绑绳前持续以 #D/F 双音节奏提示距离；越近越快；**发现 NPC 后继续响，只有绑绳成功才停**；图标从"右上角脉冲雷达点"升级为"潜艇声纳仪表"（旋转扫描臂 + 目标光点 + 脉冲波纹）；玩家身上 LED 保留作世界同步反馈；Web Audio 合成音；GM 面板"探知仪"Tab 全参数可调 |
| **P10** | **左上角 HUD 管理器** | 🟡 中 | ✅ 已完成 | 新建 `HUDTopLeft.ts` 统一管理迷宫模式左上角四项（氧气环/手动挡/音频/探知仪）；竖向等距布局 + 入场滑入动效；统一交互：短按=主操作+弹 tip（2s 自动消失，淡入淡出），氧气环额外支持长按展开详情面板；所有硬坐标 hit-test 从 `input.ts` 收口到 HUDTopLeft，`drawAudioToggle` 独立绘制已废弃 |
