...
| **P12** | **画质分档 + FPS 自适应** | 🔴 高 | ✅ 已完成 | 针对 `light.volPass` / `light.maskPass` 性能瓶颈；四档（low 0.25x / medium 0.5x / high 0.75x / ultra 1.0x）通过动态调整 WebGL glCanvas 分辨率降 fragment 工作量；同时按档位限制 VPL 上传点数（16/48/96/128），低档关闭漫散射与 NPC 体积光；自适应策略：采样 FPS 每 60 帧一个窗口，连续 2 窗口 <45 降档、连续 3 窗口 >55 升档，2s 冷却；auto 默认从 high 起步且不会自动升到 ultra（ultra 仅手动选）；GM 面板"性能"Tab 新增 8 项可调参数 |
